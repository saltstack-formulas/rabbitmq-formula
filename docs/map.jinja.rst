...
We will see a quick setup to configure the ``rabbitmq`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``rabbitmq`` formula
`````````````````````````````````````````````````````

Now, we can provides the per DNS domain name configuration files for the ``rabbitmq`` formulas under ``/srv/salt/rabbitmq/parameters/``.
...
We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/rabbitmq/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/rabbitmq/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``rabbitmq`` formula
``````````````````````````````````````````````````````

Now, we can provides the per role configuration files for the ``rabbitmq`` formulas under ``/srv/salt/rabbitmq/parameters/``.
...
We create a configuration for the role ``rabbitmq/server`` in ``/srv/salt/rabbitmq/parameters/roles/rabbitmq/server.yaml``:
...
We create another configuration for the role ``rabbitmq/client`` in ``/srv/salt/rabbitmq/parameters/roles/rabbitmq/client.yaml``:
...
Here is an example based on `rabbitmq-formula/rabbitmq/config/file.sls`_:
...
This ``sls`` file expose a ``rabbitmq`` context variable to the jinja template which could be used like this:
...
.. _rabbitmq-formula/rabbitmq/config/file.sls: https://github.com/saltstack-formulas/rabbitmq-formula/blob/master/rabbitmq/config/file.sls
